		...
		<view class="main">
			<view class="content">
				<view class="nav">
					...
					<view class="nav-list" id="life0">会员管理</view>
				...
				<view v-if="sexFlag" class="tabSex">
					<view v-for="(sexVal, index) in sex" :key="index">
						{{ sexVal.name }}
					</view>
				</view>
				<view v-if="ageFlag" class="tabSex" >
					<view v-for="(ageVal, index) in age" :key="index" @click="chooseAge(ageVal, index)">
						{{ ageVal.name }}
					</view>
				</view>
					...
					<view class="huiyuan_info"  v-for="(huiyuanVal, index) in huiyuan"  :key="index" @click="huiyuanChange(huiyuanVal)">
						<view class="lists">
							...
							<view class="huiyuan_hanzi">
								<view class="name">
									{{ huiyuanVal.name }}&nbsp;&nbsp;<lable><image class="sexImg" :src="huiyuanVal.sex=='男'?sexMale:sexFemale"></image></lable>&nbsp;<label class="pos" :style="{backgroundColor:huiyuanVal.color}">{{ huiyuanVal.posi }}</label>
	...
	export default {
		...
		data() {
			...
			return {
				...
				sexMale:this.httpUrl+'file/static/membMang/xingbie@2x.png',//性别标识
				sexFemale:this.httpUrl+'file/static/membMang/nvxing@2x.png',//性别标识
				xiajiantou:this.httpUrl+'file/static/membMang/xiajiantou@2x.png',//下箭头
				shouhuijiantou:this.httpUrl+'file/static/membMang/shouhuijiantou@2x.png',//上箭头
		...
		methods: {
			...
			choose(index){
				...
				if(_this.isChooseState == 0){
					...
					_this.sexFlag = !_this.sexFlag;
					_this.ageFlag = false;
					_this.dateFlag = false;
				}else if(_this.isChooseState == 2){
					_this.sexFlag = false;
					_this.ageFlag = !_this.ageFlag;
					_this.dateFlag = false;
				}else if(_this.isChooseState == 3){
					_this.sexFlag = false;
					_this.ageFlag = false;
			...
			// 日历组件结束
			// 选择性别
			chooseSex(sexVal, index){
				console.log(index)
				console.log(sexVal)
			},
			// 选择年龄
			chooseAge(ageVal, index){
				console.log(index)
				console.log(ageVal)
			}
		}
...
.tabSex{
	height: auto;
	margin-top: 20upx;
	box-shadow:0upx 1upx 0upx 0upx rgba(0, 0, 0, 0.13);
	background: #fff;
